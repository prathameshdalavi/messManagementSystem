...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleItemClick = (item: any) => {
    ...
    // removed Logout handling from here — Logout is rendered separately at bottom
  };
  ...
  return (
    <div
      className={`fixed top-0 left-0 h-full bg-[#2F3349] text-white z-40 transform transition-transform duration-300 w-64 md:translate-x-0 ${opensidebar ? "translate-x-0" : "-translate-x-full"
        }`}
    >
      {/* Header */}
      ...
      <div className="flex flex-col justify-start px-4 py-4 font-semibold text-lg overflow-y-auto h-full gap-2 md:gap-3">
          ...
          functionalityButtons.map((func) => (
            <button
              ...
              onClick={() => {
                ...
                if (isMobile()) toggleSidebar(); // close when a functionality selected on mobile
              }}
              className={`w-full text-center px-3 py-2 rounded transition-colors ${selectedFunctionality === func.id
        ...
        {/* Main Menu (exclude Logout here) */}
        {currentMenu === "main" && !selectedPlan &&
          sidebarItems
            .filter((it) => it.label !== "Logout")
            .map((item) => (
              <button
                key={item.label}
        ...
        <div className="mt-auto px-3 py-4">
          <button
            ...
            className="w-full px-3 py-2 rounded bg-red-600 hover:bg-red-700 text-white font-semibold"
          >
            Logout
          </button>
        </div>
