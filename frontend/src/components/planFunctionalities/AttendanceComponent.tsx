...
export const AttendanceComponent: React.FC = () => {
  ...
  const startScanner = async () => {
    ...
    try {
      ...
      if (/Permission|denied|NotAllowedError/i.test(msg)) setMessage({ type: 'error', text: 'Camera permission denied.' });
      else if (/NotFoundError|no camera/i.test(msg)) setMessage({ type: 'error', text: 'No suitable camera found.' });
      else if (/NotReadableError|track/i.test(msg)) setMessage({ type: 'error', text: 'Camera already in use.' });
      else setMessage({ type: 'error', text: `Camera error: ${msg}` });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <AnimatePresence>
        {message && (
          ...
          >
            ...
            <button onClick={clearMessage} className="text-gray-400 hover:text-gray-600"><FaTimes /></button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {showScanner && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={stopScanner} className="text-gray-400 hover:text-gray-600"><FaTimes size={20} /></button>
          </div>
          <div className="mb-4">
      ...
      {!selectedPlan?.messId?._id ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          <p className="text-gray-400 max-w-md mx-auto">Please select a mess plan to view attendance records.</p>
        </motion.div>
      ) : attendance ? (
        <div className="space-y-6">
          {/* Present Stats */}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12 bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          <p className="text-gray-400 max-w-md mx-auto">No attendance records available for this plan yet.</p>
          <button onClick={fetchAttendance} className="mt-4 text-sm text-teal-600 hover:text-teal-800 font-medium transition-colors">
            Refresh Records
          </button>
        </motion.div>
      )}
